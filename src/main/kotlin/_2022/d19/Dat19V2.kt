...
class Day19(val input: String)  {
    ...
    fun solve1(): Int = blueprints.sumOf { println("done"); BluePrintSimulator(it, 24).getMostGeodes() * it.id }

    fun solve2(): Int = blueprints.take(3).map { println("over"); BluePrintSimulator(it, 32).getMostGeodes() }.reduce(Int::times)
    ...
    class BluePrintSimulator(private val blueprint: Blueprint, private val duration: Int) {
        ...
        private fun buildClayBot(state: SimulationState): SimulationState {
            ...
            val wait = if (oresNeeded == 0)
                1
            else ceil(oresNeeded / state.oreProd.toFloat()).toInt() + 1
        ...
        private fun buildOreBot(state: SimulationState): SimulationState {
            ...
            val wait = if (oresNeeded == 0)
                1
            else ceil(oresNeeded.toFloat() / state.oreProd).toInt() + 1
...
fun main() {
    var d = Day19(File("src/main/kotlin/_2022.d19/input.txt").readText())
...
// here numbers are initialized and sorted
    ...
    println("(The operation took $timeInMillis2 ms)")
}